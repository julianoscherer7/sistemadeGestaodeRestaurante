...
def tela_pedidos(janela_principal):
    janela = tk.Toplevel(janela_principal)
    janela.title("Pedidos")
    janela.geometry("600x400")

    # Frame para o número da mesa
    frame_mesa = ttk.Frame(janela)
    frame_mesa.pack(pady=10)

    ttk.Label(frame_mesa, text="Número da Mesa:").grid(row=0, column=0, padx=5, pady=5)
    ...
    ttk.Label(frame_itens, text="Selecione os Itens:").pack()
    ...
    for item in itens:
        ...
        ttk.Checkbutton(item_frame, text=f"{item[1]} - R${item[2]:.2f}", variable=var).pack(side="left", padx=5)
    ...
    frame_total = ttk.Frame(janela)
    frame_total.pack(pady=10)

    ttk.Label(frame_total, text="Valor Total:").pack(side="left", padx=5)
    label_total = ttk.Label(frame_total, text="R$ 0.00")
    ...
    ttk.Button(janela, text="Calcular Total", command=calcular_total).pack(pady=10)
    ...
    ttk.Button(janela, text="Voltar", command=janela.destroy).pack(pady=10)
